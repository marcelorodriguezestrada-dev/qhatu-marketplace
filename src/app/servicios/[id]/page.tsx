  ...
  return (
    <div className="max-w-[560px] mx-auto px-5 py-8">
      ...
      <div className="bg-panel border border-line rounded-xl overflow-hidden mb-6">
        <div className="bg-panelalt py-8 flex justify-center text-maroon">
          <ServiceIcon kind={perfil.icono} size={48} />
        </div>
        <div className="p-6">
          <div className="font-body text-xs text-inksoft mb-1">
            {RUBROS.find((r: any) => r.id === perfil.rubro)?.label || perfil.rubro}
          </div>
          <div className="font-display text-xl font-bold text-ink mb-1">{perfil.nombre}</div>
          <div className="font-body text-sm text-inksoft mb-3">{perfil.zona}</div>
          ...
          {perfil.cantidadResenas > 0 ? (
            ...
            <div className="font-body text-xs text-inksoft mb-4">Todavía sin reseñas</div>
          )}

          {perfil.descripcion && (
            <p className="font-body text-sm text-ink mb-5">{perfil.descripcion}</p>
          )}
