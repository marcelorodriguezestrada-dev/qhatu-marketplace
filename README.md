...
## Ronda: página de producto con pestañas + tienda del vendedor

- **`/producto/[id]` rediseñado** con pestañas "Publicación" e "Info. Tienda" — foto grande, detalle del producto (talles, colores, materiales, compra mínima, rubro, hace cuánto se publicó), y una pestaña de tienda con mapa (si el vendedor cargó ubicación), horarios, tipos de venta y badge de verificado.
- **Nuevos campos de producto**: `talles`, `colores`, `materiales`, `compraMinima` — todos opcionales, se cargan desde `/vender`.
- **Perfil de tienda del vendedor ampliado** (antes solo tenía QR/CBU): ahora también dirección con ubicación real (mapa), horarios, tipos de venta (mayorista, minorista, envíos, cambios, etc.) y logo — todo se carga desde `/vender` → "Mi tienda".
- **Se dejó de mostrar el email del vendedor** en el catálogo y el detalle del producto — ahora se muestra el nombre de su tienda (y el logo, si lo cargó). Si todavía no configuró su tienda, cae al email como respaldo. Cuando el vendedor actualiza el nombre/logo de su tienda, se actualiza automáticamente en todos sus productos ya publicados.
- **Badge "Verificado"**: lo activás vos manualmente desde `/admin` para un vendedor puntual (`PATCH /api/vendedores/[id]`) — nunca es automático, para que signifique algo real.
